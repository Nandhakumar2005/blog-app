...
export default function Navbar({ posts, onFilter }: Props) {
  ...
  return (
    <nav
      ...
      "
    >
      <div className="relative z-50 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between lg:gap-8">
                ...
                >
                  {moreTags.map((tag) => (
                    <button
                      key={tag}
                      onClick={() => handleTagClick(tag)}
                      className={`block w-full px-5 py-3 text-left text-sm capitalize transition hover:bg-slate-100 sm:text-base dark:hover:bg-slate-800 ${
                        activeTag === tag
